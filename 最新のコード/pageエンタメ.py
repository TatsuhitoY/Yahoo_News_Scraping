...
def job_main():
    ...
    t_2 = a.strftime('%m-%d')
    file_name = '/Users/yoshiharatatsuhito/Desktop/T.Yoshihara/計算社会科学/Python_code/yahoo_analysis/データファイル/エンタメ {}.csv'.format(t_2)
    ...
    while True: #while文の最初の階層でcontinueが適用されると記事の情報が取得されず最後のprint文も機能せず、そのままスキップされる
        ...
        URL = "https://news.yahoo.co.jp/topics/entertainment?page={}".format(news_page_num)
        ...
        for num, url in enumerate(article_list):
          ...
          soup = BeautifulSoup(rest.text, "html.parser")
          #title取得
          title = soup.find(class_ = "sc-kYWsKJ gpvqmW")
          if title:
          ...
          #media取得
          media = soup.find(class_ = "sc-hKNIYV bVcTQx")#主要トピックの記事
          if not media:
              #サッカー速報などで、画像の上にしかメディアが掲載されていない時
              media = soup.find(class_ = "sc-eDZMvD jxRyJZ")
          if media:
          ...
          #p_time取得
          #通常の記事で、画像の上に表示されている
          p_time = soup.find(class_ = "sc-guHSqG dBkayT")
          if not p_time:
            #動画の記事
            p_time = soup.find(class_ = "sc-jrsTQk eWFjut")
          if not p_time:
            #画像と動画の無い記事
            p_time = soup.find(class_ = "sc-jvOanY ZXLYq")
          if p_time:
          ...
          cmt_num = soup.find(class_ = "sc-wzDOX iQqrut")
          if not cmt_num:
              #コメントの数が少なくて表示が青色になっている時
              cmt_num = soup.find(class_ = "sc-wzDOX bYtiKO")
          if cmt_num:
          ...
          url_text = soup.find(class_ = "sc-ipHgwE kZUceQ")
